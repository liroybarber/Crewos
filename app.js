...
/* ── NUMBER HELPERS (feature 7: integers only, no decimals) ── */
function n(v){ return Number(v)||0; }
function formatMoney(v){ return "\u20AA" + Math.round(n(v)); }
...
function logout(){
  ses=null; pin=""; lSel="owner";
  auth.signOut().catch(function(){});
  showPg("pg-welcome");
}
function changeBiz(){
  ses=null; pin=""; lSel="owner"; bizCode=null; S=null;
  localStorage.removeItem("crewos_biz");
  auth.signOut().catch(function(){});
  showPg("pg-welcome");
}
...
function defS(bizName, ownerName, sitePassword, ownerPassword, phone){
  return {
    ...
    phone: phone||"",
    nid: 3, goal: 0,
    svcs: [{id:"s1",lbl:"תספורת",price:120},{id:"s2",lbl:"זקן",price:60},{id:"s3",lbl:"שניהם",price:160}],
...
function ensureS(data){
  if(!data) return defS();
  data.emps         = data.emps||[];
  data.svcs         = data.svcs||[{id:"s1",lbl:"תספורת",price:120},{id:"s2",lbl:"זקן",price:60},{id:"s3",lbl:"שניהם",price:160}];
...
/* feature 5: get worker's personal services (falls back to global) */
function getEmpSvcs(emp){
...
/* ── REGISTRATION (feature 2: unique businessCode) ── */
async function genUniqueCode(){
  for(var i=0;i<20;i++){
    var code = String(Math.floor(1000+Math.random()*9000));
    var snap = await db.ref("businessCodes/"+code).once("value");
...
async function registerBiz(){
  ...
  var authPass  = document.getElementById("reg-auth-pass").value;
  var authPass2 = document.getElementById("reg-auth-pass2").value;
  ...
  if(authPass.length<6)                 {err.textContent="הסיסמה חייבת להיות לפחות 6 תווים";return;}
  if(authPass!==authPass2)              {err.textContent="הסיסמאות לא תואמות";return;}
  if(!/^\d{4}$/.test(ownerPass))        {err.textContent="סיסמת ניהול: 4 ספרות בלבד";return;}
  if(ownerPass!==ownerPass2)            {err.textContent="אישור סיסמת ניהול לא תואם";return;}
  if(!/^\d{4}$/.test(sitePass))         {err.textContent="קוד עסק לעובדים: 4 ספרות בלבד";return;}
  err.textContent="רושם...";
  var regBtn=document.getElementById("reg-btn"); if(regBtn)regBtn.disabled=true;
  try{
    var userCred  = await auth.createUserWithEmailAndPassword(email, authPass);
    var uid       = userCred.user.uid;
    var bizId     = uid.slice(0,8)+"_"+Date.now(); // unique business ID
    var bizCode_  = await genUniqueCode();          // unique 4-digit code for employees
    var newData   = defS(bizName, ownerName, sitePass, ownerPass, phone);
    newData.ownerEmail   = email;
    newData.ownerUid     = uid;
    newData.businessCode = bizCode_;
    await db.ref("businesses/"+bizId).set({
      bizName:bizName, ownerName:ownerName, phone:phone,
      ownerEmail:email, ownerUid:uid, businessCode:bizCode_,
      createdAt:new Date().toISOString(), data:newData
    });
    await db.ref("owners/"+uid).set({businessId:bizId, email:email});
    await db.ref("emails/"+email.replace(/\./g,",")).set(bizId);
    await db.ref("businessCodes/"+bizCode_).set(bizId); // employee entry point
    bizCode=bizId; S=newData;
    try{localStorage.setItem("crewos_biz",bizId);}catch(e){}
    try{localStorage.setItem("crewos_cache_"+bizId,JSON.stringify(S));}catch(e){}
    var sn=document.getElementById("succ-name"),sb2=document.getElementById("succ-biz"),
        sp=document.getElementById("succ-phone"),ss=document.getElementById("succ-site-pass"),
        sc=document.getElementById("succ-emp-code");
    if(sn)sn.textContent="ברוך הבא, "+ownerName+"!";
    if(sb2)sb2.textContent=bizName;
    if(sp)sp.textContent=phone;
    if(ss)ss.textContent=sitePass+" (סיסמת ניהול)";
    if(sc)sc.textContent=bizCode_+" (קוד לעובדים)";
    showPg("pg-reg-success");
  }catch(e){
    console.error("[REGISTER]",e);
    var msg=e.code==="auth/email-already-in-use"?"מייל זה כבר רשום במערכת":
            e.code==="auth/invalid-email"?"כתובת מייל לא תקינה":
            e.code==="auth/weak-password"?"הסיסמה חלשה מדי":
            "שגיאה: "+(e.message||e.code||e);
    err.textContent=msg;
    if(regBtn)regBtn.disabled=false;
  }
}

/* ── OWNER LOGIN (feature 3: email+password, Auth only) ── */
async function ownerLogin(){
  var email = (document.getElementById("owner-login-email").value||"").trim().toLowerCase();
  ...
  err.textContent="";
  if(!email||!email.includes("@")){err.textContent="נא להזין כתובת מייל";return;}
  if(!pass)                        {err.textContent="נא להזין סיסמה";return;}
  err.textContent="מתחבר..."; btn.disabled=true;
  try{
    var userCred = await auth.signInWithEmailAndPassword(email, pass);
    var uid      = userCred.user.uid;
    /* find business */
    var ownerSnap = await db.ref("owners/"+uid).once("value");
    var od = ownerSnap.val();
    var code = od&&od.businessId;
    if(!code){
      /* legacy: search by email */
      var eSnap = await db.ref("emails/"+email.replace(/\./g,",")).once("value");
      code = eSnap.val();
    }
    if(!code){err.textContent="עסק לא נמצא לחשבון זה"; btn.disabled=false; return;}
    var bSnap = await db.ref("businesses/"+code).once("value");
    var biz   = bSnap.val();
    if(!biz||!biz.data){err.textContent="נתוני עסק חסרים"; btn.disabled=false; return;}
    err.textContent=""; btn.disabled=false;
    bizCode=code; S=ensureS(biz.data);
    ...
    var msg=e.code==="auth/wrong-password"||e.code==="auth/invalid-credential"?"סיסמה שגויה":
            e.code==="auth/user-not-found"||e.code==="auth/invalid-email"?"מייל לא נמצא":
            e.code==="auth/too-many-requests"?"יותר מדי ניסיונות":
            "שגיאה: "+(e.message||e.code||e);
    err.textContent=msg; btn.disabled=false;
  }
}

/* feature 4: forgot password via email */
async function ownerForgotPassword(){
  var emailEl = document.getElementById("owner-forgot-email");
  var msg     = document.getElementById("owner-forgot-msg");
  var btn     = document.getElementById("owner-forgot-btn");
  var email   = (emailEl&&emailEl.value.trim().toLowerCase())||"";
  if(!email||!email.includes("@")){if(msg)msg.textContent="נא להזין מייל תקין";return;}
  if(btn)btn.disabled=true;
  try{
    await auth.sendPasswordResetEmail(email);
    if(msg){msg.style.color="var(--gr)";msg.textContent="נשלח מייל לאיפוס סיסמה ✓";}
  }catch(e){
    var errMsg=e.code==="auth/user-not-found"?"מייל לא נמצא במערכת":"שגיאה: "+(e.message||e.code);
    if(msg){msg.style.color="var(--re)";msg.textContent=errMsg;}
  }finally{if(btn)btn.disabled=false;}
}

/* ── EMPLOYEE LOGIN (feature 1: businessCode -> worker list -> PIN) ── */
async function empLogin(){
  var codeVal = (document.getElementById("emp-login-code").value||"").replace(/\s/g,"");
  ...
  try{
    /* feature 1: look up via businessCodes/{code} */
    var codeSnap = await db.ref("businessCodes/"+codeVal).once("value");
    var bizId    = codeSnap.val();
    /* legacy fallback: maybe code IS the bizId */
    if(!bizId){
      var directSnap = await db.ref("businesses/"+codeVal).once("value");
      if(directSnap.val()&&directSnap.val().data) bizId=codeVal;
    }
    if(!bizId){err.textContent="קוד עסק לא נמצא"; btn.disabled=false; return;}
    var bSnap = await db.ref("businesses/"+bizId).once("value");
    var biz   = bSnap.val();
    if(!biz||!biz.data){err.textContent="נתוני עסק לא נמצאו"; btn.disabled=false; return;}
    err.textContent=""; btn.disabled=false;
    bizCode=bizId; S=ensureS(biz.data);
    try{localStorage.setItem("crewos_emp_biz",bizId);}catch(e){}
    try{localStorage.setItem("crewos_cache_"+bizId,JSON.stringify(S));}catch(e){}
    /* show worker picker */
    showEmpPicker();
  }catch(e){
    console.error("[EMP LOGIN]",e);
    err.textContent="שגיאת חיבור: "+(e.code||e.message||e);
    btn.disabled=false;
  }
}

/* feature 1: show large worker selection buttons */
function showEmpPicker(){
  var emps = (S.emps||[]);
  if(!emps.length){
    alert("אין עובדים רשומים בעסק זה"); return;
  }
  var h="<div style='text-align:center;margin-bottom:20px'>";
  h+="<div style='font-size:22px;font-weight:900;color:var(--or)'>"+S.bizName+"</div>";
  h+="<div style='font-size:13px;color:var(--mu);margin-top:4px'>מי אתה?</div></div>";
  emps.forEach(function(e){
    h+="<button onclick='selectEmpWorker("+e.id+")' style='width:100%;height:64px;border-radius:14px;margin-bottom:10px;background:"+e.color+"18;border:2px solid "+e.color+";color:var(--tx);font-size:17px;font-weight:700;display:flex;align-items:center;gap:14px;padding:0 16px'>";
    ...
    h+="<div style='text-align:right'><div>"+e.name+"</div><div style='font-size:12px;color:var(--dm);font-weight:400'>"+e.role+"</div></div></button>";
  });
  var pickerEl = document.getElementById("emp-picker-content");
  if(pickerEl) pickerEl.innerHTML=h;
  showPg("pg-emp-picker");
}

var pendingEmpId = null;
function selectEmpWorker(empId){
  var emp = (S.emps||[]).find(function(e){return e.id===empId;});
  if(!emp) return;
  if(emp.pin&&emp.pin.length===4){
    /* ask PIN */
    pendingEmpId = empId;
    var pt=document.getElementById("emp-pin-name"); if(pt)pt.textContent="קוד כניסה — "+emp.name;
    var pe=document.getElementById("emp-pin-err");  if(pe)pe.textContent="";
    pin=""; rdotsEmp();
    showPg("pg-emp-pin");
  }else{
    /* no PIN needed */
...
function rdotsEmp(){
  for(var i=0;i<4;i++){var d=document.getElementById("ep"+i);if(d)d.className="dt"+(pin.length>i?" on":"");}
  var b=document.getElementById("emp-pin-btn");if(!b)return;
  b.disabled=pin.length<4; b.style.opacity=pin.length<4?".4":"1";
}
function epk(nm){if(pin.length<4){pin+=nm;rdotsEmp();}}
function epdel(){pin=pin.slice(0,-1);rdotsEmp();}
function submitEmpPin(){
  var emp=(S.emps||[]).find(function(e){return e.id===pendingEmpId;});
  if(!emp) return;
  var entered=pin.replace(/\s/g,"");
  var correct=(emp.pin||"").replace(/\s/g,"");
  if(entered===correct){
    pin=""; enterAsEmp(pendingEmpId);
  }else{
    var pe=document.getElementById("emp-pin-err");if(pe)pe.textContent="קוד שגוי";
    setTimeout(function(){var pe2=document.getElementById("emp-pin-err");if(pe2)pe2.textContent="";},1500);
    pin=""; rdotsEmp();
  }
}

function enterAsEmp(empId){
  var emp=(S.emps||[]).find(function(e){return e.id===empId;});
  if(!emp) return;
  ses={role:"emp",eid:empId};
  if(emp.pm==="chair"){
...
/* ── PICK USER (owner internal PIN screen) ── */
...
/* ── CALC (feature 7: use worker's personal services) ── */
function calc(list, emp){
  ...
  return{gross:Math.round(gross),es:Math.round(es),os:Math.round(os),hpay:Math.round(hpay)};
}
...
function closeDay(){
  ...
  sv();
  db.ref("businesses/"+bizCode+"/dailyReports/"+date).set(report).catch(function(e){console.error(e);});
  rOwner();
  alert("היום נסגר.");
}
...
function confirmUnlock(){
  var entered=(document.getElementById("unlock-pin").value||"").replace(/\s/g,"");
  var ownerPwd=(S.ownerPassword||"").replace(/\s/g,"");
  var ue=document.getElementById("unlock-err");
  if(entered!==ownerPwd){ue.textContent="סיסמה שגויה";return;}
...
function rOwner(){
  ...
  mt=Math.round(mt+ownerMonth); om=Math.round(om+ownerMonth);
...
function owTab(t){
  owT=t;
  ["today","emps","report","history","svcs"].forEach(function(x){var el=document.getElementById("t-"+x);if(el)el.style.display=x===t?"":"none";});
  ["tbt","tbe","tbr","tbh","tbs"].forEach(function(id,i){var el=document.getElementById(id);if(!el)return;var tabs=["today","emps","report","history","svcs"];el.className="tab "+(t===tabs[i]?"on":"off");});
  rOwTab(t);
}
function rOwTab(t){
  if(t==="today")rToday();
...
function rToday(){
  ...
  if(!closed) h+="<button onclick='openOwnerEntry()' style='width:100%;height:38px;border-radius:10px;margin-top:12px;background:"+(ownerTe?"var(--btn)":"rgba(232,120,42,.1)")+";border:1px solid "+(ownerTe?"var(--br)":"var(--or)")+";color:"+(ownerTe?"var(--mu)":"var(--or)")+";font-size:13px;font-weight:700'>"+(ownerTe?"עדכן יום שלי":"הזן יום שלי")+"</button>";
  h+="</div>";
  (S.emps||[]).forEach(function(e){
    ...
    if(!closed&&e.pm!=="chair") h+="<button onclick='openEntry("+e.id+")' style='width:100%;height:36px;border-radius:10px;margin-top:10px;background:"+(te?"var(--btn)":"rgba(232,120,42,.1)")+";border:1px solid "+(te?"var(--br)":"var(--or)")+";color:"+(te?"var(--mu)":"var(--or)")+";font-size:13px;font-weight:700'>"+(te?"עדכן":"הזן יום")+"</button>";
    h+="</div>";
  });
...
function rEmps(){
  ...
  (S.emps||[]).forEach(function(e){
    ...
    var isC=e.pm==="chair";
    /* feature 5: show personal services count */
    var empSvcsCount=(e.svcs||[]).length;
    h+="<div class=card><div style='display:flex;align-items:center;justify-content:space-between;margin-bottom:12px'>";
    h+="<div style='display:flex;align-items:center;gap:12px'>";
    h+="<div class=av style='width:44px;height:44px;background:"+e.color+"22;border:2px solid "+e.color+";font-size:13px;color:"+e.color+"'>"+e.av+"</div>";
    h+="<div><div style='font-weight:800;font-size:15px'>"+e.name+"</div><div style='color:var(--dm);font-size:11px'>"+e.role+(empSvcsCount?" \u00b7 "+empSvcsCount+" שירותים":"")+"</div></div></div>";
    ...
    if(!isC){
      ...
      h+="<button onclick='openEntry("+e.id+")' style='flex:2;height:40px;border-radius:12px;background:"+(hasT?"var(--btn)":e.color+"18")+";border:1px solid "+(hasT?"var(--br)":e.color)+";color:"+(hasT?"var(--mu)":e.color)+";font-size:13px;font-weight:700;"+(dayClosed?"opacity:.4;cursor:not-allowed":"")+"\'"+(dayClosed?" disabled":"")+">"+( hasT?"עדכן יום":"הזן יום")+"</button>";
    }else{
...
function rHistory(){
  ...
  sortedDays.forEach(function(d){Object.values(days[d]).forEach(function(row){if(row.emp.id==="owner"){mTotal+=n(row.entry.total);mOwner+=n(row.entry.total);}else{var r=calc([row.entry],row.emp);mTotal+=n(r.gross);mSalary+=n(r.es);mOwner+=n(r.os);mCancel+=n(row.entry.cancels);}});});
  ...
  sortedDays.forEach(function(d){
    ...
    rows.forEach(function(row){
      var empColor=row.emp.color||"#E8782A";
      ...
      else{var r=calc([row.entry],row.emp);gross=n(r.gross);sal=n(r.es);ownerShare=n(r.os);}
      dayTotal+=gross;
      h+="<div style='display:flex;justify-content:space-between;padding:10px 0;border-bottom:1px solid var(--br)'>";
      h+="<div style='display:flex;align-items:center;gap:10px'>";
      h+="<div class=av style='width:34px;height:34px;background:"+empColor+"22;border:2px solid "+empColor+";font-size:10px;color:"+empColor+"'>"+ini(row.emp.name)+"</div>";
      h+="<div><div style='font-weight:700;font-size:13px'>"+row.emp.name+"</div><div style='color:var(--dm);font-size:11px'>"+sm+"</div></div></div>";
      ...
      h+="<div style='font-size:14px;font-weight:900;color:var(--or)'>"+formatMoney(gross)+"</div></div></div>";
    });
...
/* ── SETTINGS (feature 5: per-worker services) ── */
function rSvcs(){
  /* global services */
  var h="<div class=sl>שירותים גלובליים</div><div class=card style='margin-bottom:6px'>";
  h+="<div style='font-size:11px;color:var(--mu);margin-bottom:12px'>ברירת מחדל לעובדים שאין להם שירותים אישיים</div>";
  ...
  h+="</div><button onclick='openAddSvc()' class='bg2' style='width:100%;height:44px;font-size:13px;margin-bottom:20px'>+ הוסף שירות גלובלי</button>";
  /* passwords */
  h+="<div class=sl>הגדרות בעל עסק</div><div class=card>";
  if(S.businessCode){
    h+="<div style='display:flex;justify-content:space-between;align-items:center;margin-bottom:14px;padding-bottom:14px;border-bottom:1px solid var(--br)'>";
    h+="<span style='font-size:13px;color:var(--mu)'>קוד לעובדים</span>";
    h+="<span style='font-size:20px;font-weight:900;color:var(--or);letter-spacing:4px'>"+S.businessCode+"</span></div>";
  }
  h+="<div class=fw><div class=fl>שנה סיסמת ניהול</div><input type='password' id='new-owner-pass' maxlength='4' inputmode='numeric' placeholder='••••'></div>";
  h+="<div class=fw><div class=fl>אשר סיסמת ניהול</div><input type='password' id='new-owner-pass2' maxlength='4' inputmode='numeric' placeholder='••••'></div>";
  h+="<div id='owner-pass-msg' style='font-size:12px;min-height:16px;margin-bottom:8px'></div>";
  h+="<button onclick='changeOwnerPass()' class=bg style='width:100%;height:44px;border-radius:12px;font-size:14px'>שמור סיסמת ניהול</button></div>";
...
function saveOwnerEntry(){
  ...
  var tot=Math.round(svcs.reduce(function(s,x){return s+n(x.cnt)*n(x.price);},0));
...
/* ── EMPLOYEE DASHBOARD (feature 6: worker sees only self) ── */
function rEmp(){
  ...
  var tc=te?n(te.cancels):0,tca=all.reduce(function(s,x){return s+n(x.cancels);},0);
  var avg=all.length?Math.round(mo.es/all.length):0;
  ...
  sorted.forEach(function(en){
    ...
    hist+="<div class=card><div style='display:flex;justify-content:space-between;align-items:center'><div><div style='font-weight:700;font-size:13px'>"+ds+"</div><div style='color:var(--dm);font-size:11px;margin-top:2px'>"+sm+(h?" "+h+"h":"")+(c2?" ביטולים:"+c2:"")+(t?" טיפ:"+t:"")+"</div></div><div style='text-align:left'><div style='color:var(--or);font-weight:900;font-size:16px'>"+formatMoney(er.es)+"</div><div style='color:var(--dm);font-size:10px'>"+en.totalSvcs+" שירותים</div></div></div></div>";
  });
  var eh=document.getElementById("emp-hist"); if(eh)eh.innerHTML=hist;
}

/* ── ENTRY MODAL (feature 5: use worker's personal services) ── */
function openEntry(eid){
  var resolvedEid=eid>0?eid:(ses&&ses.role==="emp"?ses.eid:null); if(!resolvedEid)return;
  if(isDayClosed(td())&&(!ses||ses.role!=="owner")){alert("יום נסגר \u2013 לא ניתן לערוך");return;}
  eEid=resolvedEid;
  var e=(S.emps||[]).find(function(x){return x.id===eEid;}); if(!e)return;
  var empSvcs=getEmpSvcs(e); /* feature 5 */
  var ex=((S.entries&&S.entries[eEid])||{})[td()];
  eCtr={};eCan=0;eTip=0;eHrs=0;
  empSvcs.forEach(function(s){eCtr[s.id]=ex?n(((ex.svcs||[]).find(function(x){return x.id===s.id;})||{cnt:0}).cnt):0;});
...
function rEntrySvcs(){
  ...
  var esvcs=document.getElementById("e-svcs"); if(esvcs)esvcs.innerHTML=html;
  updTot();
}
...
function updTot(){
  ...
  var empSvcs=getEmpSvcs(e);
  var cnt=0,amt=0;
  empSvcs.forEach(function(s){cnt+=n(eCtr[s.id]);amt+=n(eCtr[s.id])*n(s.price);});
  var hp=e.pm==="hourly"?Math.round(n(eHrs)*n(e.hr)):0;
  var el;
  ...
  el=document.getElementById("e-tamt");if(el)el.textContent=formatMoney(Math.round(amt));
...
function saveEntry(){
  var e=(S.emps||[]).find(function(x){return x.id===eEid;})||{};
  var empSvcs=getEmpSvcs(e);
  /* feature 5: snapshot of service name+price at time of entry */
  var svcs=empSvcs.map(function(s){return{id:s.id,lbl:s.lbl,cnt:n(eCtr[s.id]),price:n(s.price)};});
  var ts=svcs.reduce(function(s,x){return s+n(x.cnt);},0);
  var tot=Math.round(svcs.reduce(function(s,x){return s+n(x.cnt)*n(x.price);},0));
...
/* ── EDIT EMP (feature 5: per-worker services tab) ── */
function openEditModal(id){
  if(!ses||ses.role!=="owner"){alert("רק בעל עסק יכול לערוך עובדים");return;}
...
function rEdMdl(){
  ...
  if(sp)rEdSp();
  /* feature 5: personal services section */
  rEdEmpSvcs(e);
}
...
/* feature 5: render personal services editor */
function rEdEmpSvcs(e){
  var empSvcs = e.svcs||[];
  var h="<div style='margin-top:16px;padding-top:16px;border-top:1px solid var(--br)'>";
  h+="<div style='font-size:12px;color:var(--or);font-weight:700;margin-bottom:10px'>שירותים אישיים לעובד זה</div>";
  if(!empSvcs.length){
    h+="<div style='font-size:12px;color:var(--dm);margin-bottom:10px'>משתמש בשירותים הגלובליים</div>";
  }else{
    empSvcs.forEach(function(s){
      h+="<div style='display:flex;align-items:center;gap:8px;margin-bottom:8px'>";
      h+="<input type='text' id='esvc-name-"+s.id+"' value='"+s.lbl+"' style='flex:2;height:40px;font-size:13px'>";
      h+="<input type='number' id='esvc-price-"+s.id+"' value='"+s.price+"' min='0' style='flex:1;height:40px;font-size:13px;text-align:center'>";
      h+="<button onclick='delEmpSvc(\""+s.id+"\")' style='width:36px;height:36px;border-radius:8px;background:rgba(239,68,68,.1);border:1px solid rgba(239,68,68,.2);color:var(--re);font-size:14px'>✕</button></div>";
    });
  }
  h+="<button onclick='addEmpSvc()' style='width:100%;height:38px;border-radius:10px;background:rgba(232,120,42,.08);border:1px solid var(--or);color:var(--or);font-size:13px;font-weight:700;margin-top:4px'>+ הוסף שירות אישי</button></div>";
...
function addEmpSvc(){
  ...
  if(!e.svcs)e.svcs=[];
  e.svcs.push({id:"es"+Date.now(),lbl:"שירות חדש",price:0});
  rEdMdl();
}
function delEmpSvc(sid){
  var e=(S.emps||[]).find(function(x){return x.id===edId;}); if(!e||!e.svcs)return;
  e.svcs=e.svcs.filter(function(s){return s.id!==sid;});
  rEdMdl();
}

function saveEdit(){
  if(!ses||ses.role!=="owner"){alert("רק בעל עסק יכול לערוך");return;}
  var e=(S.emps||[]).find(function(x){return x.id===edId;}); if(!e)return;
  var en=document.getElementById("ed-name"),er=document.getElementById("ed-role"),ep=document.getElementById("ed-pin");
  ...
  if(e.svcs&&e.svcs.length){
    e.svcs.forEach(function(s){
      ...
      var np2=document.getElementById("esvc-price-"+s.id);
      if(nl)s.lbl=nl.value.trim()||s.lbl;
      if(np2)s.price=n(parseInt(np2.value));
    });
  }
...
function deleteEmp(id){
  ...
  if(!confirm("למחוק את "+e.name+"?\nהדיווחים ישמרו.")){return;}
  S.emps=(S.emps||[]).filter(function(x){return x.id!==id;});
  sv();
  db.ref("businesses/"+bizCode+"/data/emps").set(S.emps).catch(function(e){console.error(e);});
  closeM("m-edit"); rOwner();
}

/* ── ADD EMP ── */
function openAddModal(){
  if(!ses||ses.role!=="owner"){alert("רק בעל עסק יכול להוסיף עובד");return;}
  addM="percent";
  var f=["ad-name","ad-role","ad-pin"];
...
function saveAdd(){
  ...
  (S.emps=S.emps||[]).push(e); sv(); closeM("m-add"); rOwner();
}
...
function openEditSvc(id){
  ...
  if(st)st.textContent="עריכת שירות";if(sn)sn.value=s.lbl;if(sp)sp.value=s.price;if(se)se.textContent="";
  openM("m-svc");
}
function saveSvc(){
...
(function(){
  function bindBtn(id,fn){
    var el=document.getElementById(id);
    if(el)el.onclick=fn;
    else console.error("[INIT] not found:",id);
  }
  bindBtn("btn-go-owner-login", function(){ showPg("pg-owner-login"); });
  bindBtn("btn-go-emp-login",   function(){ showPg("pg-emp-login"); });
  bindBtn("btn-go-register",    function(){ showPg("pg-register"); });
  bindBtn("owner-login-btn",    function(){ ownerLogin(); });
  bindBtn("owner-login-back",   function(){ showPg("pg-welcome"); });
  bindBtn("emp-login-btn",      function(){ empLogin(); });
  bindBtn("emp-login-back",     function(){ showPg("pg-welcome"); });
  ...
  bindBtn("emp-picker-back",    function(){ showPg("pg-emp-login"); });
  bindBtn("emp-pin-btn",        function(){ submitEmpPin(); });
  bindBtn("emp-pin-back",       function(){ showEmpPicker(); });

  /* Auto-login for owner via Firebase Auth */
  auth.onAuthStateChanged(function(user){
    if(!user) return;
    db.ref("owners/"+user.uid).once("value").then(function(snap){
      var od=snap.val();
      var code=od&&od.businessId;
      if(!code){
        db.ref("emails/"+user.email.replace(/\./g,",")).once("value").then(function(es){
          code=es.val(); if(code) loadBizForOwner(code);
        });
        return;
      }
      loadBizForOwner(code);
    }).catch(function(e){console.error("[AUTO-LOGIN]",e);});
  });

  function loadBizForOwner(code){
    try{var c=localStorage.getItem("crewos_cache_"+code);if(c){S=ensureS(JSON.parse(c));bizCode=code;}}catch(e){}
    db.ref("businesses/"+code).once("value").then(function(snap){
      var biz=snap.val();
      if(biz&&biz.data){S=ensureS(biz.data);bizCode=code;try{localStorage.setItem("crewos_cache_"+code,JSON.stringify(S));}catch(e){}}
      if(S&&bizCode){ses={role:"owner"};rOwner();showPg("pg-owner");}
    }).catch(function(e){console.error("[LOAD BIZ]",e);});
  }
})();
